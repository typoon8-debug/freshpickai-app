...
## 진행 현황 (2026-05-22 업데이트 → Task 064 완료 · Phase 4.5 v0.3a Sprint 5 진행 중)
...
| **Phase 4 잔여: 키즈 모드** (Task 031) | 🔜 v0.3a 포함 | — |
| **Phase 4.5: v0.3a AI 채팅 업그레이드** (Task 064~069) | 🔄 Sprint 5 진행 중 | — |
...
| F014 | 키즈·청소년 모드 | P1 | 🔜 Task 069 (v0.3a) |
...
| F032 | AI 채팅 3계층 맥락 메모리 | P0 | 🔜 Task 065 (v0.3a) |
| F033 | 인텐트 기반 버튼 피드백 채널 | P0 | 🔜 Task 066 (v0.3a) |
| F034 | AI 채팅 커머스 API 연동 | P0 | 🔜 Task 067 (v0.3a) |
| F035 | 음성 입력 AI 채팅 Prototype | P1 | 🔜 Task 068 (v0.3a) |
...
> **Sprint 5 · P0 — 2026-05-16 기준 신규 스프린트**
> AI 채팅 맥락 이해 정확도 향상 + 버튼 피드백 + 커머스 API 연동 + 음성 입력 Prototype.
> **구현 순서**: 버그 수정 → 공통 기반(DB/타입) → F032 메모리 → F033 버튼 → F034 커머스 → F035 음성
...
- [ ] **메모리 조회 유틸리티** (`src/lib/chat/memory/retrieve.ts`):
...
- [ ] **메모리 저장 유틸리티** (`src/lib/chat/memory/store.ts`):
...
- [ ] **Chat Route Handler 수정** (`src/app/api/chat/route.ts`):
  - 요청 수신 → `retrieveMemoryContext()` 호출 → 메모리 컨텍스트를 시스템 프롬프트에 삽입
  - 메모리 컨텍스트 프롬프트 포맷: `[기억 요약] ... [최근 대화] ... [현재 질문]`
  - 응답 완료 후 `saveRawMessage()` user + assistant 양쪽 저장
  - 세션 종료 감지 시 비동기 `saveSessionSummary()` + `upsertMemoryItems()` 호출 (응답 블록 안 함)
- [ ] **메모리 TTL 정리 Cron** (`src/app/api/cron/cleanup-chat-memory/route.ts`):
  - `DELETE FROM fp_chat_message_raw WHERE created_at < NOW() - INTERVAL '30 days'`
  - `vercel.json` cron 등록: 매일 오전 3시 (한국 기준)
- [ ] **메모리 컨텍스트 디버그 UI** (개발 모드 전용, `src/components/chat/MemoryDebugPanel.tsx`):
  - `process.env.NODE_ENV === 'development'` 시에만 렌더링
  - 현재 요청에 삽입된 메모리 컨텍스트 3계층 확인 패널 (접힘 기본)

**테스트 체크리스트**:
- [ ] 1회차 대화 후 새 세션에서 이전 식재료 선호 기억 여부 확인 (Playwright)
- [ ] `fp_chat_message_raw` 30일 초과 레코드 TTL 삭제 확인
- [ ] pgvector 검색 응답 시간 200ms 이내 (p95) 확인

**완료 기준**: 이전 세션에서 언급한 "비건 선호" 발화 → 다음 세션 추천에 반영 확인, 메모리 저장 DB 레코드 확인
...
- [ ] **LLM 응답 스키마 수정** (`src/lib/chat/schema.ts`):
  - `ChatResponseSchema` (Zod): `{ message: z.string(), intents: z.array(ChatActionIntentSchema).max(4).optional(), context: z.object({ itemIds: z.array(z.string()).optional(), cardId: z.string().optional() }).optional() }`
  - `ChatActionIntentSchema`: `{ action: z.nativeEnum(ChatActionEnum), label: z.string().max(20), payload: z.record(z.unknown()).optional() }`
- [ ] **Chat Route Handler 응답 형식 수정** (`src/app/api/chat/route.ts`):
  - `generateObject()` + `ChatResponseSchema` 사용 → `{ message, intents, context }` JSON 반환
  - 시스템 프롬프트에 버튼 생성 규칙 추가:
    ```
    [버튼 생성 규칙]
    - 사용자 의도에 따라 최대 4개의 intent를 생성할 것
    - 버튼은 직접 만들지 말고 intent Enum만 반환
    - 상품 추천 시: ADD_TO_WISHLIST, ADD_TO_CART, VIEW_CARD
    - 결제 준비 시: INITIATE_PAYMENT, VIEW_CARD
    - 예/아니오 확인 시: CONFIRM_YES, CONFIRM_NO
    ```
- [ ] **Action JSON Renderer** (`src/components/chat/ActionButtonRenderer.tsx`):
  - `intents: ChatActionIntent[]` props 수신 → 버튼 배열 렌더링
  - 각 action Enum에 따른 스타일 분기:
    - `ADD_TO_CART` → 주색상 (Mocha Mousse), 장바구니 아이콘
    - `ADD_TO_WISHLIST` → 아웃라인, 하트 아이콘
    - `INITIATE_PAYMENT` → 성공색, 결제 아이콘
    - `CONFIRM_YES/NO` → 그린/레드 쌍 배치
    - `VIEW_CARD` → 뉴트럴, 외부링크 아이콘
  - 버튼 탭 → `onActionSelect(intent)` 콜백 실행
- [ ] **AI 채팅 컴포넌트 통합** (`src/components/chat/ChatMessage.tsx` 또는 동등 위치):
  - assistant 메시지 하단에 `ActionButtonRenderer` 배치
  - intents 없을 때 렌더링 스킵 (일반 텍스트 답변과 공존)
- [ ] **버튼 탭 핸들러** (`src/components/chat/ChatContainer.tsx`):
  - `handleActionSelect(intent)` → `intent.action`에 따라 Task 067 API 호출 또는 라우팅 분기
  - `VIEW_CARD` → `router.push('/cards/[cardId]')`
  - `SEARCH_MORE` → 채팅 입력창에 검색어 자동 삽입 후 전송
  - `CONFIRM_YES/NO` → 다음 AI 메시지 트리거 (맥락 유지)

**테스트 체크리스트**:
- [ ] "오늘 저녁 삼겹살 어때요?" 질의 시 ADD_TO_CART + VIEW_CARD 버튼 2개 렌더링 확인 (Playwright)
- [ ] 버튼 탭 후 정확한 API 호출 또는 라우팅 동작 확인
- [ ] intents 없는 일반 답변 시 버튼 미표시 확인

**완료 기준**: AI 답변 하단에 버튼 정상 렌더링, 버튼 탭 시 올바른 action 핸들러 실행 확인
...
- [ ] **찜 추가 API 확장** (`src/app/api/wishlist/route.ts`):
  - 기존 wishlist Server Action을 REST API Route Handler로 노출 (AI 채팅에서 직접 호출 가능)
  - `POST /api/wishlist` `{ itemId: string }` → `fp_wishlist` INSERT + 낙관적 UI 갱신용 응답
  - `DELETE /api/wishlist/[itemId]` → 찜 취소
  - 인증: `Authorization: Bearer {supabase_access_token}` 헤더 검증
- [ ] **장바구니 API 확장** (`src/app/api/cart/route.ts`):
  - `POST /api/cart` `{ storeItemId, qty, cardId? }` → `fp_cart_item` INSERT or UPDATE
  - `PATCH /api/cart/[itemId]` `{ qty: number }` → 수량 변경 (qty ≤ 0 시 자동 삭제)
  - `DELETE /api/cart/[itemId]` → 개별 항목 삭제
  - 재고 확인 (`v_store_inventory_item.is_in_stock`) + 품절 시 `409 Conflict` 반환
- [ ] **결제 시작 API** (`src/app/api/payment/initiate/route.ts`):
  - `POST /api/payment/initiate` `{ cartItemIds?: string[] }` → 토스페이먼츠 결제 위젯 초기화에 필요한 `{ orderId, amount, orderName }` 반환
  - cartItemIds 미제공 시 전체 장바구니 기준
  - 응답에 결제 페이지 deeplink URL 포함 (`/checkout?orderId=...`)
- [ ] **채팅 버튼 핸들러 API 연결** (`src/components/chat/ChatContainer.tsx`):
  - `ADD_TO_WISHLIST` → `fetch('/api/wishlist', { method: 'POST', body: { itemId: intent.payload.itemId } })` + 성공 토스트
  - `ADD_TO_CART` → `fetch('/api/cart', { method: 'POST', ... })` + 장바구니 카운트 TanStack Query 무효화 (`qk.cart()`)
  - `UPDATE_CART` → `fetch('/api/cart/[itemId]', { method: 'PATCH', ... })`
  - `REMOVE_FROM_CART` → `fetch('/api/cart/[itemId]', { method: 'DELETE' })`
  - `INITIATE_PAYMENT` → `fetch('/api/payment/initiate')` → `router.push(data.checkoutUrl)`
- [ ] **실시간 상태 피드백** (채팅 UI):
  - API 호출 중 버튼 로딩 스피너 표시 (버튼 disabled 처리)
  - 성공: 버튼 아래 인라인 성공 메시지 ("장바구니에 담았어요 ✓")
  - 실패(품절 등): 인라인 에러 메시지 + 버튼 재활성

**테스트 체크리스트**:
- [ ] AI 채팅 "오늘 삼겹살 재료 담아줘" → ADD_TO_CART 버튼 탭 → `fp_cart_item` DB 레코드 생성 확인 (Playwright)
- [ ] 품절 상품 ADD_TO_CART 시 에러 메시지 표시 확인
- [ ] INITIATE_PAYMENT 탭 후 결제 페이지 이동 확인
- [ ] TanStack Query `qk.cart()` 무효화 → 장바구니 카운트 뱃지 갱신 확인

**완료 기준**: AI 채팅 버튼으로 장바구니 담기 → DB 확인, 결제 페이지 이동 정상 동작 확인
...
- [ ] **음성 인식 훅** (`src/hooks/useSpeechRecognition.ts`):
...
- [ ] **음성 입력 버튼** (`src/components/chat/VoiceChatButton.tsx`):
...
- [ ] **파형 애니메이션** (`src/components/chat/SoundWaveAnimation.tsx`):
  - Framer Motion 4개 바 높이 bounce 애니메이션 (stagger 0.1s)
  - 인식 중에만 표시
- [ ] **채팅 입력 컴포넌트 통합** (`src/components/chat/ChatInput.tsx`):
  - `VoiceChatButton` 배치, `useSpeechRecognition` 훅 연결
  - `interimTranscript` → 입력창 플레이스홀더에 실시간 표시 (회색 텍스트)
  - `transcript` 확정 → 입력창 value 세팅 → 자동 제출 (`handleSubmit()`)
- [ ] **접근성 처리**:
  - `aria-label="음성으로 입력하기"` 마이크 버튼
  - `role="status"` 실시간 인식 텍스트 영역

**테스트 체크리스트**:
- [ ] 마이크 버튼 탭 → "오늘 저녁 뭐 먹을까?" 발화 → 채팅 입력창에 텍스트 자동 삽입 확인 (수동 테스트)
- [ ] Web Speech API 미지원 환경에서 마이크 버튼 비활성화 + 툴팁 표시 확인
- [ ] `npm run check-all` 통과 확인

**완료 기준**: 음성 발화 → 텍스트 변환 → AI 채팅 자동 전송 동작 확인 (Chrome/Edge 기준), 미지원 브라우저 graceful degradation 확인
...
- [ ] **연령 그룹 필터 훅** (`src/hooks/useKidsFilter.ts`):
  - `fp_cards_by_ai_tags` RPC 호출 — 초등: `ai_tags && ARRAY['간식','디저트','어린이','무알콜']`, 청소년: `ai_tags && ARRAY['트렌디','글로벌','홈시네마']`
  - `family_member.age_group` (ELEMENTARY | TEEN) 기반 필터 분기
  - `useKidsStore` 연동 — 선택된 아이 프로필 기준 자동 필터 적용
- [ ] **키즈 탭 UI** (`src/components/home/KidsTabView.tsx`):
  - 초등 탭: 간식팩·K-디저트·드라마 카드 필터 그리드
  - 청소년 탭: 트렌드·글로벌·홈시네마 카드 필터 그리드
  - 아이 프로필 선택 드롭다운 (가족 구성원 중 아이 타입만)
- [ ] **아이 별점 UI** (`src/components/family/KidsRating.tsx`):
  - 별 1~5개 탭 인터랙션 (터치 최적화, 44px hit target)
  - `rateCard(cardId, memberId, rating)` Server Action 호출
- [ ] **별점 저장 Server Action** (`src/lib/actions/kids/rating.ts`):
  - `rateCard(cardId, memberId, rating)` → `fp_family_vote` INSERT (vote_type: 'KIDS_RATING', score: 1~5)
  - `getKidsPreferredCards(groupId)` → 평균 별점 ≥ 4.0 카드 우선 노출 가중치 업데이트
- [ ] **우리가족 보드 "우리 아이 선호" 섹션 연동** (`src/components/family/KidsPreferenceSection.tsx`):
  - 별점 저장 후 가족 보드 해당 섹션 실시간 갱신 (TanStack Query `qk.family()` invalidate)
  - 다음 주 홈 카드 섹션 가중치 반영 (가중치 컬럼: `fp_card_section_item.weight` 업데이트)

**완료 기준**: 키즈 탭 카드 필터 동작, 아이 별점 저장 → DB 확인, 우리가족 보드 선호 섹션 갱신 확인
...
| **Task 065** | AI 채팅 3계층 메모리 백엔드 | F032: 원문 TTL·요약·memory_items 파이프라인 | P0 |
| **Task 066** | 인텐트 버튼 피드백 채널 | F033: Action Enum + LLM 프롬프트 + Action JSON Renderer | P0 |
| **Task 067** | AI 채팅 커머스 API 연동 | F034: 찜/장바구니/결제 API + 버튼 핸들러 연결 | P0 |
| **Task 068** | 음성 입력 AI 채팅 Prototype | F035: Web Speech API + 파형 애니메이션 | P1 |
| **Task 069** | 키즈·청소년 모드 구현 | F014: 연령별 카드 필터 + 아이 별점 시스템 | P1 |
