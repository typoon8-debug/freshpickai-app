...
## 진행 현황 (2026-05-17 업데이트 → Task 055/056/057/059 완료 + 인앱 알림함 + 핫픽스 3건 + F032 메모리 보강 + 모바일 성능 최적화 1~3단계 완료)
...
| 포인트 | 파일 | 변경 내용 | 예상 효과 |
|--------|------|-----------|-----------|
| **P5 Firebase 동적 임포트** | `src/app/(main)/layout.tsx` | `FcmInitializer` + `NotificationProvider` → `dynamic({ ssr: false })` | 초기 JS 15~25% 감소 |
