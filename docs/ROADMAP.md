...
- [ ] ESLint + Prettier + Husky pre-commit 훅 설정 (`npm run check-all` 게이트 구성)
...
- [ ] **TanStack Query 설정** (`src/lib/query-client.ts`): `QueryClient` 기본 설정, `src/lib/query-keys.ts` 쿼리 키 컨벤션 (`qk.cards()`, `qk.card()`, `qk.daily()`, `qk.family()`, `qk.cart()`, `qk.memos()`)
- [ ] **API 클라이언트** (`src/lib/api.ts`): `fetch` 래퍼, 인증 헤더 자동 주입, 에러 포맷 파싱 (`{ error: { code, message } }`)
- [ ] **공통 유틸리티** (`src/lib/utils.ts`): `cn()` (clsx + tailwind-merge), `formatPrice()`, `formatDate()`, `formatDeliveryTime()`
- [ ] **환경 변수 유효성 검사** (`src/lib/env.ts`): 필수 환경 변수 미설정 시 빌드 타임 에러
...
#### Task 016: Supabase 데이터베이스 스키마 및 마이그레이션 — 우선순위

**목적**: PRD 데이터 모델 전체를 Supabase PostgreSQL에 구현 + pgvector + RLS 정책

**구현 항목**:

- [ ] **핵심 테이블 마이그레이션**: `customer`, `customer_preference`, `menu_card`, `card_ingredient`, `family_group`, `family_member`, `family_vote` 테이블 생성 (PRD 데이터 모델 기준)
...
- [ ] **customer_preference 컬럼 확장** (F019 BP5): `cook_time_pref TEXT NULL`, `budget_range TEXT NULL`, `onboarding_completed_at TIMESTAMPTZ NULL`, `onboarding_skipped_at TIMESTAMPTZ NULL` 컬럼 추가
- [ ] **dish_recipe HNSW 인덱스**: `dish_recipe.embedding` cosine HNSW 인덱스 (목표 200ms 이하 검색)
- [ ] **커머스 테이블**: `cart`, `cart_item`, `order`, `order_detail` 테이블 생성
- [ ] **AI/RAG 테이블**: `tenant_item_ai_detail` (chunk_type 10종, embedding vector(1536), persona_tags, ACTIVE/STALE/REVIEW_NEEDED 상태), `ai_query_cache` (query_embedding, response_payload JSONB, expires_at 7일 TTL) 테이블 생성
- [ ] **섹션 관리 테이블**: `card_section`, `card_section_item` 테이블 생성 (F015)
- [ ] **pgvector 확장 + HNSW 인덱스**: `CREATE EXTENSION vector`, `tenant_item_ai_detail.embedding` HNSW cosine 인덱스 (목표: 200ms 이하 검색), `pg_trgm` 확장
- [ ] **RLS 정책 추가**: `card_note`는 작성자 + 같은 가족 그룹원 읽기 가능, `dish_recipe`는 ACTIVE만 일반 사용자 읽기 가능, REVIEW_NEEDED는 작성자 + 운영자만. 모든 테이블 Row Level Security 활성화
- [ ] **뷰 및 헬퍼 함수**: `v_store_inventory_item` 뷰, `match_items()` pgvector 유사도 검색 함수
- [ ] **Supabase TypeScript 타입 재생성** (`src/lib/supabase/database.types.ts`): Supabase CLI로 자동 생성
...
#### Task 017: CORRECTION_DICT 오타 보정 사전 구축
...
- [ ] **타입 정의** (`src/lib/utils/correction-dict.types.ts`): `DictEntry { wrong: string; correct: string; source: string; }`, `DictSource`, `DictMeta` 타입
- [ ] **통합 사전 파일** (`src/data/correction-dict/dict.json`): 식재료·장보기 도메인 100~250개 오타 페어 (예: "계란" 오타 → "계란", "냉이" 오타 → "냉이")
- [ ] **도메인 allowlist** (`src/data/correction-dict/domain-allowlist.txt`): 식재료·요리 표제어 약 3,000개
- [ ] **applyCorrection() 함수** (`src/lib/utils/correction-dict.ts`): longest-key-first greedy replace 알고리즘, 단일 진입점
- [ ] **6단계 검증**: 자모거리 검증, 충돌 검증, 순환 검증, 재귀 안정성 검증, 금칙어 검증, 빈도 검증
- [ ] **ETL 파이프라인** (`src/scripts/build-correction-dict.ts`): GitHub 공개 데이터셋(#1 hunspell-dict-ko allowlist + #2 github-typo-corpus 한국어 페어) 파싱 → 라이선스 필터링(CC BY 4.0 이상만) → dict.json 생성
- [ ] **단위 테스트** (`src/lib/utils/correction-dict.test.ts`): 주요 오타 보정 케이스, 재귀 안정성, 금칙어 필터 테스트
