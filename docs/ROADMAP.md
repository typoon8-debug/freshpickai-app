...
## 진행 현황 (2026-05-16 업데이트 → Task 030 완료)
...
| **Phase 3: AI 기능 + RAG 시스템** (Task 024~030, 037~041) | 🔄 진행 중 | — |
...
- [ ] **자기보강 루프 트리거** (`src/lib/actions/notes/self-improve.ts`): `helpful_count >= 5` AND `ai_consent=true` → Claude Haiku 4.5 LLM Judge 사실성 평가 (≥ 4/5 통과 시 `fp_dish_recipe`에 `source='user_note'`·`status='REVIEW_NEEDED'` UPSERT, 자동 ACTIVE 승격 금지)
...
- [ ] **재료 메타 시드** (`src/scripts/seed-ingredient-meta.ts`): 주요 재료 약 100종에 손질법·계량 힌트·대체 재료 시드
- [ ] **IngredientMetaBlock 컴포넌트** 완성 (Task 008 자리 표시 → 실제 데이터 연결)
- [ ] **F003 substitutes 우선 참조** (`src/lib/ai/tools/search-items.ts` recipe 모드): `fp_ingredient_meta.substitutes` JSONB + `v_store_inventory_item.ai_tags` 교차 참조로 대체 재료 자동 제안
- [ ] **사용자 노트 → substitutes 자동 병합 큐**: helpful_count ≥ 10 도달 시 운영자 검수 큐 등록
...
- [ ] **카카오 SDK 통합** (`src/lib/share/kakao.ts`): `Kakao.Share.sendCustom` 호출, 백업: Web Share API
- [ ] **OG 메타 동적 생성 라우트** (`src/app/cards/[id]/opengraph-image.tsx`): Next.js `ImageResponse` — 메뉴 이미지 + 건강 스코어 + 브랜드 워터마크
- [ ] **비로그인 카드 미리보기 페이지** (`src/app/cards/[id]/preview/page.tsx`): 카드 정보 + "FreshPickAI 시작하기" CTA
- [ ] **ShareButton 컴포넌트** 완성 (Task 008 자리 표시 → 실 동작)
- [ ] **공유 추적 이벤트** (PostHog `card_shared`): 카드 ID, 공유 채널, 신규 가입 전환 측정
...
- [ ] **가이드 키워드 시스템** (`src/data/wizard-guide-keywords.ts`): 카드테마별 추천 메뉴명 패턴, 재료별 손질법·대체 재료 자동완성 (`fp_ingredient_meta` 연동)
- [ ] **Step3 강화**: 재료 입력 시 `prep_method`·`substitutes` 함께 입력 가능한 펼침 영역, Task 052의 `matchIngredientToStoreItemAction` 통합
- [ ] **검수 신청 버튼**: "공식 카드섹션에 신청" 버튼 → `review_status='pending'`
- [ ] **AI 학습 동의** (선택): 동의 시 승인된 카드 레시피가 `fp_dish_recipe`에 `source='user_note'`·`status='REVIEW_NEEDED'`로 등록
...
- [ ] **온보딩 진입 가드 미들웨어**: `customer_preference.onboarding_completed_at IS NULL` 체크 → `/auth/onboarding` 리디렉션
- [ ] **마이페이지 "온보딩 다시 보기"**: `onboarding_skipped_at` 무시하고 재진입 가능
