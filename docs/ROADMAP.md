...
| **Task 022: 장보기 메모 4-step 파싱 파이프라인 (F012)** | ✅ **완료** | **2026-05-13** |
| **Task 023: 카드 만들기 + 섹션 관리 API (F013, F015)** | ✅ **완료** | **2026-05-13** |
...
- [x] **카드 만들기 Server Action** (`src/lib/actions/cards/create.ts`): `createCardAction(values)` — 위자드 결과를 `fp_menu_card`에 저장, `card_theme → category` 자동 파생 (snack_pack→snack, cinema_night→cinema, 나머지→meal), `review_status: 'private'`으로 저장 ✅ 2026-05-13
- [x] **cards/new/page.tsx 실제 연동**: `handleSave` 스텁 → `createCardAction` 호출, 에러 처리 + 홈 리다이렉트 ✅ 2026-05-13
- [x] **섹션 관리 Server Actions** (`src/lib/actions/sections/index.ts`): ✅ 2026-05-13
  - `getSectionsAction()` — 신규 사용자 10종 공식 섹션 자동 시드 포함
  - `createSectionAction(name)` — 커스텀 섹션 추가
  - `updateSectionAction(sectionId, name)` — 섹션 이름 변경
  - `deleteSectionAction(sectionId)` — 섹션 삭제 (본인 섹션만)
  - `reorderSectionsAction(sections)` — 순서 일괄 업데이트
  - `toggleAiAutoFillAction(sectionId, aiAutoFill)` — AI 자동채움 토글
- [x] **sections/page.tsx DB 연동**: 하드코딩 `DEFAULT_SECTIONS` → 실제 DB CRUD 연동, 낙관적 UI 업데이트 ✅ 2026-05-13
- [x] **신규 사용자 공식 섹션 자동 시드**: `getSectionsAction()` 호출 시 섹션 0개이면 10종(흑백요리사·오늘뭐먹지·가족레시피·드라마식탁·혼웰빙·제철한상·세계한상·K디저트·간식박스·홈시네마) 자동 생성 ✅ 2026-05-13

**테스트 결과** (2026-05-13):
```
✅ Playwright: 섹션 관리 페이지 최초 방문 → 10종 공식 섹션 자동 생성 확인 (DB 조회 일치)
✅ Playwright: "오늘뭐먹지" 위로 이동 → sort_order 0으로 갱신, 흑백요리사 sort_order 1로 갱신
✅ Playwright: "주말특선" 커스텀 섹션 추가 → fp_card_section INSERT + 목록 표시 확인
✅ Playwright: 카드 만들기 위자드 4단계 (제철한상 / 담백한·저칼로리·가족식사 / 시금치1단·15000원) → 카드 만들기 클릭
✅ Supabase SQL: fp_menu_card 레코드 생성 확인 (name: "담백한 · 저칼로리", card_theme: "seasonal", review_status: "private")
✅ npm run typecheck 0 errors, npm run build 성공
```

**완료 기준**: ✅ 카드 생성 DB 저장, 섹션 CRUD 동작, 신규 사용자 공식 섹션 자동 시드 확인 (2026-05-13)
