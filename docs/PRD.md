...
**목적**: 공식 10종 + 사용자 커스텀 카드섹션으로 오늘 한 끼를 선택하고, AI에게 변형을 요청하고, 가족과 투표한 뒤, 재료를 한꺼번에 새벽배송 받는 AI 큐레이팅 장보기의 의사결정 마찰을 없앰
...
6. 완료 → 배송 예약 확인 (파트너 배송 예정, 익일 새벽 도착 목표) → 가족 무비나이트 기록 저장
```
...
> `searchItems` 도구는 `mode` 파라미터(`recipe|item`)로 레시피 RAG와 상품 RAG를 내부 분기 처리 — 5-tool 구조 유지.
...
|----|--------|------|--------------|------------|
| **F001** | 10종 카드메뉴 시스템 | 식사형 7종 + 비식사형 3종 카드를 3-Layer 구조로 제공 (카드메뉴 → 건강·가격 인프라 → AI Agent) | 검색 없이 30초 메뉴 결정 — 서비스 핵심 차별점 | 카드메뉴 홈 |
...
| **F014** | 키즈·청소년 모드 | P8 초등(간식팩·K-디저트·드라마 카드 필터)·P9 중고생(트렌드·글로벌·홈시네마) 전용 카드 뷰, 아이 별점 → 다음 주 카드 반영 | 가족 전 구성원 진입, 방과후 간식팩 수요 | 카드메뉴 홈, 우리가족 보드 |
| **F015** | 카드섹션 커스터마이징 | 공식 10종 탭과 사용자 생성 탭을 자유롭게 관리. **생성**(섹션명 자유 입력, 예: "내맘대로 메뉴", "주는데로 먹어") · **삭제**(공식 탭 포함, 해당 사용자에게만 숨김) · **명칭 변경**(공식 탭 이름도 수정 가능) · **순서 변경**(드래그앤드롭). **AI 자동 채움**은 섹션 타입이 아닌 모든 탭에 ON/OFF 가능한 속성 — OFF 시 카드 직접 선택, ON 시 페르소나 컨텍스트 기반 AI가 해당 섹션 맥락에 맞는 카드 자동 생성 | 사용자 홈 완전 개인화 — 재방문율·앱 체류 시간 향상 | 카드메뉴 홈, 내 섹션 관리 |
...
| **F019** | 온보딩 5장 슬라이드 (BP5) | 로그인 직후 5장 슬라이드 — ① "오늘 뭐 먹지를 30초에" (카드 10종 미리보기) ② "AI가 우리 가족에 맞춰" (F003 데모) ③ "우리가족 보드로 함께 결정" (F011 데모) ④ "키즈 모드로 아이도 참여" (F014 데모) ⑤ "내 취향 설정" — **가구 인원·웰빙 목표·요리시간·예산** 4가지 폼. 모든 단계 [건너뛰기] 명시. 마이페이지 재진입 가능 | 신규 사용자 핵심 가치 전달, 페르소나 RAG 핵심 컨텍스트 수집 | 로그인 |
...
| **주요 기능** | • 자연어 입력창 ("모아달에게 물어보세요…")<br>• 9 페르소나 컨텍스트 주입 (가구·웰빙 목표·요리시간·예산·최근 주문)<br>• pgvector 5-tool 호출 (searchItems·getUserContext·getInventory·addToCart·addToMemo — searchItems는 dish_recipe 레시피 RAG 1차 + tenant_item_ai_detail 상품 RAG 2차 내부 분기)<br>• 스트리밍 텍스트 + 구조화 메뉴 카드 3개 반환<br>• **담기** 버튼 (응답 카드에서 직접 장바구니 담기) |
...
| **주요 기능** | • **탭 생성** — 섹션명 자유 입력 후 새 탭 추가 (예: "내맘대로 메뉴", "주는데로 먹어", 또는 사용자가 원하는 임의 이름)<br>• **탭 삭제** — 공식 10종 탭·사용자 생성 탭 모두 삭제 가능 (공식 탭 삭제는 해당 사용자에게만 숨김, 전체 시스템 영향 없음)<br>• **탭 명칭 변경** — 공식 탭 이름 포함 모든 탭 이름 수정 가능 (예: "셰프스 테이블" → "오늘의 도전 요리")<br>• **탭 순서 변경** — 드래그앤드롭으로 홈 탭 노출 순서 재배치<br>• **AI 자동 채움 토글** — 공식 탭·사용자 탭 구분 없이 모든 탭에 개별 적용 가능<br>&nbsp;&nbsp;&nbsp;- OFF: 사용자가 직접 공식 카드 또는 내 카드(F013)를 선택해 탭에 추가<br>&nbsp;&nbsp;&nbsp;- ON: 페르소나 컨텍스트·섹션명 맥락 기반으로 AI가 탭 카드를 자동 생성·갱신<br>• **저장** 버튼 → 카드메뉴 홈 탭에 즉시 반영 |
...
### tenant_item_ai_detail (AI RAG 지식 베이스 — 10종 카드 chunk 저장)
| 필드 | 설명 | 타입/관계 |
|------|------|----------|
| ai_detail_id | 고유 식별자 | UUID |
| tenant_item_id | 연결 상품 | → tenant_item_master |
| chunk_type | 청크 유형 10종 ※ | TEXT |
...
> ※ `chunk_type` 10종: `chef_recipe`·`nutrition_plan`·`family_recipe`·`drama_recipe`·`honwell_bowl`·`seasonal_ingredient`·`global_plate`·`k_dessert`·`snack_pack`·`cinema_pairing`

### family_group (가족 그룹) ← Critical Fix #1
| 필드 | 설명 | 타입/관계 |
|------|------|----------|
...
| section_type | 섹션 출처 (official·custom) — 공식·사용자 생성 구분 | TEXT |
| section_code | 공식 섹션 원본 코드 (official만, 예: chef_table) — 명칭 변경 후에도 원본 식별용 | TEXT |
| section_name | 사용자에게 보이는 탭명 (기본값=공식명, 자유 수정 가능) | TEXT |
| ai_auto_fill | AI 자동 채움 여부 — 공식·커스텀 탭 모두 ON/OFF 가능 | BOOLEAN |
| ai_persona_tags | AI 자동 채움 조건 태그 (ai_auto_fill=true 시 사용) | TEXT[] |
...
- **Tailwind CSS** + **shadcn/ui (new-york)** — 토큰 기반 빠른 구현
- **Lucide React** — 아이콘
- **next-themes** — 다크 모드
...
- **Layer 2**: **Anthropic Claude Sonnet 4.6** — 9 페르소나 응답·Tool Calling (한국어 품질 우수) / **Anthropic Claude Haiku 4.5** — 분류·요약·캐시 키 정규화
- **Layer 3**: **OpenAI text-embedding-3-small** (`openai`) — 1536차원 한국어 임베딩

### 🗄️ 백엔드 & 데이터베이스

- **Supabase** — Auth + PostgreSQL + Storage + Realtime + **pgvector 0.8.x** (HNSW cosine, 200ms 이하) ← Critical Fix #2
...
- **Playwright E2E** — CI `npm run check-all` 게이트: 핵심 5~10 시나리오 (로그인·홈·카드상세·담기·결제 골든 패스). 9 페르소나 × 카드메뉴 10종 전체 300건 골든 셋은 주간·배포 전 별도 실행 (CI 게이트에는 포함하지 않음)
