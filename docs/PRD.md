...
| **F015** | 카드섹션 커스터마이징 | 공식 테마 탭과 사용자 생성 탭을 자유롭게 관리. **생성**(섹션명 자유 입력, 예: "내맘대로 메뉴", "주는데로 먹어") · **삭제**(공식 탭 포함, 해당 사용자에게만 숨김) · **명칭 변경**(공식 탭 이름도 수정 가능) · **순서 변경**(드래그앤드롭). **AI 자동 채움**은 섹션 타입이 아닌 모든 탭에 ON/OFF 가능한 속성 — OFF 시 카드 직접 선택, ON 시 페르소나 컨텍스트 기반 AI가 해당 섹션 맥락에 맞는 카드 자동 생성 | 사용자 홈 완전 개인화 — 재방문율·앱 체류 시간 향상 | 카드메뉴 홈, 내 섹션 관리 |
...
| **F019** | 온보딩 5장 슬라이드 (BP5) | 로그인 직후 5장 슬라이드 — ① "메뉴를 30초에" (다양한 테마 카드 미리보기) ② "AI가 우리 가족에 맞춰" (F003 데모) ③ "우리가족 보드로 함께 결정" (F011 데모) ④ "키즈 모드로 아이도 참여" (F014 데모) ⑤ "내 취향 설정" — **가구 인원·웰빙 목표·요리시간·예산** 4가지 폼. 모든 단계 [건너뛰기] 명시. 마이페이지 재진입 가능 | 신규 사용자 핵심 가치 전달, 페르소나 RAG 핵심 컨텍스트 수집 | 로그인 |
...
- 공동구매 D-day 모델 (카드 재료 일괄 담기 트랙과 별도)
