...
import type { FpAddress } from "@/lib/actions/address";
...
export function AddressBlock({ address, onChangeRequest }: Props) {
  return (
    <div className="border-line overflow-hidden rounded-lg border bg-white">
      <div className="border-line flex items-center justify-between border-b px-4 py-3">
        ...
        <button
          ...
          className="text-mocha-600 flex items-center text-xs"
        >
      ...
      <div className="px-4 py-3">
        {address ? (
          <>
            <p className="text-ink-900 text-sm font-semibold">{address.addressName}</p>
            <p className="text-ink-500 mt-1 text-sm">
              {address.address}
              {address.addrDetail ? ` ${address.addrDetail}` : ""}
            </p>
          ...
          <button type="button" onClick={onChangeRequest} className="text-ink-400 text-sm">
            배송지를 선택해주세요 →
          </button>
        )}
