...
import { MOVIE_GENRES } from "@/lib/constants/family";
import type { MovieGenre } from "@/lib/constants/family";
import { sendMovieNightNotification } from "@/lib/actions/push/send";
...
응답은 반드시 JSON 형식으로 작성하세요.`,
  ...
  // Poll 연결: result_card_id 업데이트
  if (options?.pollId) {
    const admin = createAdminClient();
    await admin
      .from("fp_poll")
      .update({
        result_card_id: adultCardId,
        status: "closed",
        closed_at: new Date().toISOString(),
      })
      .eq("poll_id", options.pollId);
  }

  // FCM 알림: 가족 전원에게 무비나이트 카드 생성 완료 알림
  sendMovieNightNotification({
    groupId,
    cardTitle: generated.adult.name,
    cardId: adultCardId,
  }).catch(() => null);
